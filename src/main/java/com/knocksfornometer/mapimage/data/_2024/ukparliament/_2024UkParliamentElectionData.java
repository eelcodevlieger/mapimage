package com.knocksfornometer.mapimage.data._2024.ukparliament;
...
public class _2024UkParliamentElectionData implements ElectionDataLoader {
	...
	private static final String INPUT_DATA_FILE_RESULTS_BY_CANDIDATE = "src\\main\\resources\\election_data\\2024\\uk_parliament\\HoC-GE2024-results-by-candidate.xlsx";
	private static final String INPUT_DATA_FILE_RESULTS_BY_CONSTITUENCY = "src\\main\\resources\\election_data\\2024\\uk_parliament\\HoC-GE2024-results-by-constituency.xlsx";
...
//			final String partyName = row.getCell(7).getStringCellValue();
			final String partyIdentifier = row.getCell(7).getStringCellValue();
			final double numVotes = row.getCell(17).getNumericCellValue();
...
//			int percentage = (int) Math.round(100.0 * voteShare);
	...
	private static Map<String, Long> loadElectorateSizeData(final Workbook workBook) {
		...
		for(Row row : constituencySheet) {
			...
			final String constituencyName = row.getCell(2).getStringCellValue();
			final Long totalElectorate = Math.round(row.getCell(12).getNumericCellValue());
