...
import com.knocksfornometer.mapimage.data.ConstituencyMapping;
...
public class _2015GuardianElectionData {
	...
	private Candidate[] createCandidates(JsonDataConstituency constituency, JsonDataCandidate[] candidateData, Map<String, String> partyColorMapping) {
		...
		for (int i = 0; i < candidateData.length; i++) {
			JsonDataCandidate candidateJson = candidateData[i];
			int percentage = (int)(candidateJson.percentage / 100d * percentageTurnout);
			String partyCode = candidateJson.party;
			Candidate candidate = new Candidate(partyColorMapping, partyCode, percentage);
